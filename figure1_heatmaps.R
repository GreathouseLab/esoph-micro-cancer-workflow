...
otu.dat <- analysis.dat %>% filter(sample_type != "0") %>%
  ...
  dplyr::filter(AverageRelativeAbundance>=avgRelAbundCutoff | OTU %like% "Campy") %>%
...
plot.dat <- analysis.dat %>% filter(sample_type != "0") %>%
  ...
  dplyr::ungroup() %>%
  dplyr::filter(aveAbund>=avgRelAbundCutoff | OTU %like% "Campy")  %>%
...
otu.dat <- analysis.dat %>% filter(sample_type != "0") %>%
  ...
  dplyr::filter(AverageRelativeAbundance>=avgRelAbundCutoff | 
                  otu2 %like% "Campylobacter concisus" |
                  otu2 %like% "Streptococcus spp." |
                  otu2 %like% "Prevotella melaninogenica" |
                  otu2 %like% "Fusobacterium nucleatum") %>%
  dplyr::arrange(desc(AverageRelativeAbundance))
...
# save data
write.csv(plot.dat, file="figure1_heatmap_tcga_rnaseq_raw_otu_data.csv")
...
plt_hmap1 <- ggplot(hmd, 
  ...
  geom_tile() +
  #facet_wrap(.~sample_type)+
  scale_fill_gradient2("Rel. Abund.",trans="sqrt", high=myColor[1], mid = myColor[75], low=myColor[100], midpoint = 0.10, breaks=c(0, 0.05, 0.10, 0.15, 0.20)) +
  scale_x_continuous(breaks = spd$x_center, 
                     labels = spd$ID, 
                     expand = c(0, 0)) + 
  # For the y axis, alternatively set the labels as: gene_position_table$gene
...
otu.dat <- analysis.dat %>% filter(sample_type != "0") %>%
  dplyr::group_by(otu2) %>%
  dplyr::summarise(AverageRelativeAbundance=mean(Abundance, na.rm=T))%>%
  dplyr::filter(AverageRelativeAbundance>=avgRelAbundCutoff  | otu2 %like% "Campylobacter concisus") %>%
  dplyr::arrange(desc(AverageRelativeAbundance))
...
plot.dat <- analysis.dat %>% filter(sample_type != "0") %>%
  ...
  dplyr::mutate(aveAbund=mean(Abundance, na.rm=T)) %>%
  dplyr::ungroup() %>%
  dplyr::filter(aveAbund>=avgRelAbundCutoff  | OTU %like% "Campylobacter concisus") %>%
  dplyr::mutate(ID = as.factor(Patient_ID),
                Abundance = ifelse(is.na(Abundance), 0, Abundance)) %>%
  dplyr::select(sample_type, Phylum, Genus, ID, Abundance, aveAbund) %>%
...
# save data
write.csv(plot.dat, file="figure1_heatmap_tcga_wgs_otu_data.csv")
