...
plot.dat <- dat.wgs.s %>% filter(sample_type != "0") %>%
...
p <- plot_grid(plt_dendr, plt_hmap1, plt_hmap2,plt_hmap3, align = 'h', rel_widths = c(0.3, 0.4, 0.4, 1),ncol = 4)
p
ggsave("output/slide-10-heatmap.pdf", plot=p, units="in", width=10, height=6)
ggsave("output/slide-10-heatmap.png", plot=p, units="in", width=10, height=6)
