...
plot.dat <- dat.rna.s %>% filter(sample_type != "0") %>%
...
p <- plot_grid(plt_dendr, plt_hmap1, plt_hmap2,plt_hmap3, align = 'h', rel_widths = c(0.3, 0.6, 0.2, 1),ncol = 4)
p
ggsave("output/slide-9-heatmap.pdf", plot=p, units="in", width=15, height=8)
ggsave("output/slide-9-heatmap.png", plot=p, units="in", width=15, height=8)
