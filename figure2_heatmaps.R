...
analysis.dat <- dat.16s.s # insert dataset to be used in analysis
...
plot.dat <- analysis.dat %>% filter(sample_type != "0") %>%
  ...
  dplyr::ungroup() %>%
  dplyr::filter(
    OTU %like% "Campylobacter concisus" |
      OTU %like% "Streptococcus spp." |
      OTU %like% "Prevotella melaninogenica" |
      OTU %like% "Fusobacterium nucleatum"
  )  %>%
...
# save data
write.csv(plot.dat, file="figure2_heatmap_nci_raw_otu_data.csv")
...
p
if(save.plots == T){
  ggsave(paste0("figure2_heatmap_nci_",save.Date,".pdf"), plot=p, units="in", width=7, height=4)
  ggsave(paste0("figure2_heatmap_nci_",save.Date,".png"), plot=p, units="in", width=7, height=4)
}
...
plot.dat <- analysis.dat %>% filter(sample_type != "0") %>%
  ...
  dplyr::ungroup() %>%
  dplyr::filter(  OTU %like% "Campylobacter concisus" |
                  OTU %like% "Streptococcus spp." |
                  OTU %like% "Prevotella melaninogenica" |
                  OTU %like% "Fusobacterium nucleatum") %>%
...
# save data
write.csv(plot.dat, file="figure2_heatmap_tcga_rnaseq_raw_otu_data.csv")
...
plt_hmap1 <- ggplot(hmd, 
  ...
  geom_tile() +
  #facet_wrap(.~sample_type)+
  scale_fill_gradient2("Rel. Abund.",trans="sqrt", high=myColor[1], mid = myColor[75], low=myColor[100],midpoint = 4e-04) +
  scale_x_continuous(breaks = spd$x_center, 
                     labels = spd$ID, 
                     expand = c(0, 0)) + 
  # For the y axis, alternatively set the labels as: gene_position_table$gene
...
if(save.plots == T){
  ggsave(paste0("figure2_heatmap_tcga_rnaseq_",save.Date,".pdf"), plot=p, units="in", width=7, height=4)
  ggsave(paste0("figure2_heatmap_tcga_rnaseq_",save.Date,".png"), plot=p, units="in", width=7, height=4)
}
...
# save data
write.csv(plot.dat, file="figure2_heatmap_tcga_wgs_otu_data.csv")
...
if(save.plots == T){
  ggsave(paste0("figure2_heatmap_tcga_wgs_",save.Date,".pdf"), plot=p, units="in", width=7, height=4)
  ggsave(paste0("figure2_heatmap_tcga_wgs_",save.Date,".png"), plot=p, units="in", width=7, height=4)
}
