...
prevalenceThreshold <- 0.01*(phyloseq::nsamples(phylo.data.nci.umd))
prevalenceThreshold
...
```{r WGS-4-phylum, out.width="100%", out.height="100%" }
...
```{r WGS-4-rel-abund-2, out.width="100%", out.height="100%" }

plot_abundance = function(physeq, title = "", Facet = "Class", ylab="Abundance"){
  mphyseq = phyloseq::subset_taxa(physeq, Phylum %in% "p__Bacteroidetes")
...
```{r WGS-4-rel-abund-3, out.width="100%", out.height="100%" }

plot_abundance = function(physeq, title = "", Facet = "Class", ylab="Abundance"){
  mphyseq = phyloseq::subset_taxa(physeq, Phylum %in% "p__Firmicutes")
  mphyseq <- phyloseq::psmelt(mphyseq)
  mphyseq <- subset(mphyseq, Abundance > 0)
  ggplot(data = mphyseq, aes(x=tissue, y=Abundance)) +
    geom_violin(fill = NA) +
...
```{r WGS-4-rel-abund-4, out.width="100%", out.height="100%" }
...
```{r WGS-4-rel-abund-5, out.width="100%", out.height="100%" }

plot_abundance = function(physeq, title = "", Facet = "Class", ylab="Abundance"){
  mphyseq = phyloseq::subset_taxa(physeq, Phylum %in% "p__Proteobacteria")
  mphyseq <- phyloseq::psmelt(mphyseq)
  mphyseq <- subset(mphyseq, Abundance > 0)
  ggplot(data = mphyseq, aes(x=tissue, y=Abundance)) +
    geom_violin(fill = NA) +
