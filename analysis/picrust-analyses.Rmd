...
```{r ec-full-plot, warning=F, error=F, message=F, fig.dim=c(12,7), out.height="100%", out.width="100%"}
...
results.out <- as.data.frame(matrix(ncol=7, nrow=length(DESCRIPTIONS)))
colnames(results.out) <- c("Description", "Parameter", "Est", "SE", "p", "ll", "ul")
...
#for(i in 1:5){ 
  ...
  results.out[i, 2] <- "tumor"
  results.out[i, 3:5] <- fit.sum$coefficients[2, c(1,2,4)]
  results.out[i, 6:7] <- confint(fit0)[2,]
  
}

results.out$fdr.p <- p.adjust(results.out$p, method="fdr")
...
```{r ko-full-plot, warning=F, error=F, message=F, fig.dim=c(12,7), out.height="100%", out.width="100%"}
...
```{r path-full-plot, warning=F, error=F, message=F, fig.dim=c(12,7), out.height="100%", out.width="100%"}
